...
def eval_video(data):
    ...
    input_var = torch.autograd.Variable(
        data.view(-1, length, data.size(2), data.size(3)), volatile=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--crop_fusion_type", type=str, default="avg", choices=["avg", "max", "topk"]
    )
    ...
    net = TSN(args.modality, consensus_type=args.crop_fusion_type,
    ...
    data_loader = torch.utils.data.DataLoader(
        TSNDataSet(
            ...
            image_tmpl="img_{:05d}.jpg"
            if args.modality == "RGB"
            else args.flow_prefix + "flow_{}_{:05d}.jpg",
    ...
    for i, (data, video_path) in enumerate(data_loader):
        ...
        ft_path = os.path.join(
            args.output_dir, video_path[0].split(os.sep)[-1] + ".pkl"
        )
        if args.modality == "RGB":
            length = 3
        elif args.modality == "Flow":
