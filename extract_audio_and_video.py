...
def pipline(video_path, output_dir, fps, sr, duration_target):
    ...
    os.system(
        f"ffmpeg -i {video_path} -loglevel error -f wav -vn -y {P.join(ori_audio_dir, audio_name)}"
    )
    ...
    duration = execCmd(
        f"ffmpeg -i {P.join(ori_audio_dir, audio_name)}  2>&1 | grep 'Duration' | cut -d ' ' -f 4 | sed s/,//"
    )
    ...
    os.system(
        "ffmpeg -stream_loop {} -i {} -loglevel error -c copy -fflags +genpts -y {}".format(
            n_repeat,
            P.join(align_video_dir, video_name),
            P.join(repeat_video_dir, video_name),
        )
    )
    ...
    os.system(
        "ffmpeg -ss 0 -t {} -i {} -loglevel error -c:v libx264 -c:a aac -strict experimental -b:a 98k -y {}".format(
            duration_target,
            P.join(repeat_video_dir, video_name),
            P.join(cut_video_dir, video_name),
        )
    )
    ...
    os.system(
        "ffmpeg -i {} -loglevel error -f wav -vn -y {}".format(
            P.join(cut_video_dir, video_name), P.join(
                cut_audio_dir, audio_name)
        )
    )
    ...
    os.system(
        "ffmpeg -i {} -loglevel error -ac 1 -ab 16k -ar {} -y {}".format(
            P.join(cut_audio_dir, audio_name), sr, P.join(
                sr_audio_dir, audio_name)
        )
    )
    ...
    os.system(
        "ffmpeg -y -i {} -loglevel error -r {} -c:v libx264 -strict -2 {}".format(
            P.join(cut_video_dir, video_name), fps, P.join(
                fps_audio_dir, video_name)
        )
    )
